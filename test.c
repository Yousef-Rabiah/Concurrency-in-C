...
char* test_send_correctness() {
    ...
    mu_assert("test_send_correctness: Testing channel values failed", string_equal(peek_buffer(channel->buffer, 0), "Message1"));
    mu_assert("test_send_correctness: Testing channel values failed", string_equal(peek_buffer(channel->buffer, 1), "Message2"));
    mu_assert("test_send_correctness: Testing channel values failed", new_args_2.out == GENERIC_ERROR);
...
char* test_receive_correctness() {
    ...
    */
    ...
    mu_assert("test_receive_correctness: Testing channel return failed", data_rec[0].out == GENERIC_ERROR);
    mu_assert("test_receive_correctness: Testing channel return failed", data_rec[1].out == GENERIC_ERROR);
...
char* test_channel_close_with_send() {
    ...
    */
    ...
    // XXX: All the threads should return in finite amount of time otherwise it will be stuck here
    for (size_t i = 0; i < SEND_THREAD; i++) {
        pthread_join(send_pid[i], NULL);    
    }
...
char* test_channel_close_with_receive() {
    ...
    // XXX: All calls should immediately return after close otherwise close implementation is incorrect
    for (size_t i = 0; i < RECEIVE_THREAD; i++) {
        pthread_join(rec_pid[i], NULL); 
    }
...
char* test_response_time() {
    print_test_details(__func__, "Testing send/receive response time (takes around 2-3 minutes)");
    ...
    */
    ...
    for (int i = 0; i < ITERS; i++) {
        ...
        sem_wait(&done);
        t = getTime() - t;
        mu_assert("test_response_time: Incorrect status", data_rec.out == SUCCESS);
        mu_assert("test_response_time: Incorrect status", out == SUCCESS);
        mu_assert("test_response_time: Incorrect message", string_equal(data_rec.data, "Message"));
    ...
    mu_assert("test_response_time: Avg response time for send/receive is higher than 0.002", avg_response_time < 0.002);
    ...
    for (int i = 0; i < ITERS; i++) {
        ...
        sem_wait(&done);
        t = getTime() - t;
        mu_assert("test_response_time: Incorrect status", data_send.out == SUCCESS);
        mu_assert("test_response_time: Incorrect status", out == SUCCESS);
        mu_assert("test_response_time: Incorrect message", string_equal(data, "Message"));
...
char* test_select() {
    ...
    */
    ...
    usleep(10000);
    mu_assert("test_select: It isn't blocked as expected", args.out == GENERIC_ERROR);

    channel_send(channel[2], "Message1");
    sem_wait(&done);

    // XXX: Code will be stuck here if the select didn't return
    pthread_join(pid, NULL);
    ...
    usleep(10000);
    mu_assert("test_select: It isn't blocked as expected", args_1.out == GENERIC_ERROR);
...
char* test_select_response_time() {
    print_test_details(__func__, "Testing select response time (takes around 2-3 minutes)");
    ...
    */
    ...
    for (int i = 0; i < ITERS; i++) {
        ...
        uint64_t t = getTime();
        enum channel_status out = channel_send(channel[2], "Message1");
        sem_wait(&done);
        t = getTime() - t;
        mu_assert("test_select_response_time: Incorrect selected index", args.index == 2);
        mu_assert("test_select_response_time: Incorrect status", args.out == SUCCESS);
        mu_assert("test_select_response_time: Incorrect status", out == SUCCESS);
        mu_assert("test_select_response_time: Incorrect message", string_equal(args.select_list[2].data, "Message1"));
    ...
    double avg_response_time = convertTimeToSeconds(total_time) / (double)ITERS;
    mu_assert("test_select_response_time: Avg response time for select is higher than 0.002", avg_response_time < 0.002);
    ...
    for (int i = 0; i < ITERS; i++) {
        ...
        void* data;
        uint64_t t = getTime();
        enum channel_status out = channel_receive(channel[0], &data);
        sem_wait(&done);
        t = getTime() - t;
        mu_assert("test_select_response_time: Incorrect selected index", args_1.index == 0);
        mu_assert("test_select_response_time: Incorrect status", args_1.out == SUCCESS);
        mu_assert("test_select_response_time: Incorrect status", out == SUCCESS);
        mu_assert("test_select_response_time: Incorrect message", string_equal(data, (i == 0) ? "Message" : "Message4"));
...
char* test_cpu_utilization_send() {
    print_test_details(__func__, "Testing CPU utilization for send API (takes around 2 minutes)");
    ...
    mu_assert("test_cpu_utilization_send: CPU Utilization is higher than required", args.data < 500000);
...
char* test_cpu_utilization_receive() {
    print_test_details(__func__, "Testing CPU utilization for receive API (takes around 2 minutes)");
    ...
    mu_assert("test_cpu_utilization_receive: CPU Utilization is higher than required", args.data < 500000);
...
char* test_cpu_utilization_select() {
    print_test_details(__func__, "Testing CPU utilization for select API (takes around 2 minutes)");
    ...
    mu_assert("test_cpu_utilization_select: CPU Utilization is higher than required", args.data < 500000);
...
char* test_stress() {
    print_test_details(__func__, "Stress Testing for all functions including select (can take some time)");
...
char* test_stress_send_recv() {
    print_test_details(__func__, "Stress Testing for send/recv without select (takes around 10 seconds)");
...
char* test_cpu_utilization_overall() {
    print_test_details(__func__, "Testing overall CPU utilization (takes around 2 minutes)");
    ...
    usleep(10000);
    mu_assert("test_cpu_utilization_overall: It isn't blocked as expected", args1.out == GENERIC_ERROR);

    channel_send(channel[1], "Message1");
    
    // XXX: Code will be stuck here if the select didn't return
    pthread_join(pid_1, NULL);
    mu_assert("test_cpu_utilization_overall: Returned value doesn't match", args1.index == 0);
    mu_assert("test_cpu_utilization_overall: Returned value doesn't match", string_equal(args1.select_list[0].data, "Message1"));
    ...
    long double result = (end.tv_sec - start.tv_sec)*1000000L + end.tv_usec - start.tv_usec + (end_s.tv_sec - start_s.tv_sec)*1000000L + end_s.tv_usec - start_s.tv_usec;
    mu_assert("test_cpu_utilization_overall: CPU Utilization is higher than required", result < 100000000);
...
char* test_for_too_many_wakeups() {
    ...
    /* This test checks the CPU utilization of all the send, receive APIs.
     */
    size_t THREADS = 100;
    ...
    for (size_t i = 0; i < THREADS; i++) {
        ...
        sem_wait(&done);
        usleep(10000);
        mu_assert("test_for_too_many_wakeups: Incorrect status", out == SUCCESS);
    }
    ...
    long double result = (end.tv_sec - start.tv_sec)*1000000L + end.tv_usec - start.tv_usec + (end_s.tv_sec - start_s.tv_sec)*1000000L + end_s.tv_usec - start_s.tv_usec;
    mu_assert("test_for_too_many_wakeups: CPU Utilization is higher than required", result < 2000000);

    for (size_t i = 0; i < THREADS; i++) {
        pthread_join(pid[i], NULL);
        mu_assert("test_for_too_many_wakeups: Incorrect status", args[i].out == SUCCESS);
        mu_assert("test_for_too_many_wakeups: Incorrect message", string_equal(args[i].data, "Message"));
    }
...
char* test_select_and_non_blocking_send(size_t capacity) {
    ...
    // To wait sometime before we check select is blocking now
    usleep(10000);
    mu_assert("test_select_and_non_blocking_send: It isn't blocked as expected", args.out == GENERIC_ERROR);
...
char* test_select_and_non_blocking_receive(size_t capacity) {
    ...
    mu_assert("test_select_and_non_blocking_receive: It isn't blocked as expected", args.out == GENERIC_ERROR);
...
char* test_select_and_non_blocking_receive_size1() {
    print_test_details(__func__, "Testing select and non-blocking receive");
    return test_select_and_non_blocking_receive(1);
}

char* test_select_and_non_blocking_send_size1() {
    print_test_details(__func__, "Testing select and non-blocking send");
...
char* test_select_with_select(size_t capacity) {
    ...
    // To wait sometime before we check select is blocking now
    usleep(10000);
    mu_assert("test_select_with_select: It isn't blocked as expected", args.out == GENERIC_ERROR);
...
char* test_select_with_select_size1() {
    print_test_details(__func__, "Testing select with select");
...
char* test_select_with_same_channel_size1() {
    print_test_details(__func__, "Testing select with same channel");
...
char* test_select_with_send_receive_on_same_channel_size1() {
    print_test_details(__func__, "Testing select with send/recv on same channel");
...
char* test_select_with_duplicate_channel_size1() {
    print_test_details(__func__, "Testing select with duplicate operations on same channel");
...
test_t tests[] = {{"test_initialization", test_initialization},
                  ...
                  {"test_stress_send_recv", test_stress_send_recv},
                  ...
                  {"test_select_and_non_blocking_send_size1", test_select_and_non_blocking_send_size1},
                  {"test_select_and_non_blocking_receive_size1", test_select_and_non_blocking_receive_size1},
                  {"test_select_with_select_size1", test_select_with_select_size1},
                  {"test_select_with_same_channel_size1", test_select_with_same_channel_size1},
                  {"test_select_with_send_receive_on_same_channel_size1", test_select_with_send_receive_on_same_channel_size1},
                  {"test_select_with_duplicate_channel_size1", test_select_with_duplicate_channel_size1},
                  ...
                  {"test_cpu_utilization_overall", test_cpu_utilization_overall},
